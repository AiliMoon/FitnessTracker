...
import com.example.android.fitnesstracker.services.Polyline
import com.example.android.fitnesstracker.services.TrackingService
...
class TrackingFragment : Fragment(R.layout.fragment_tracking) {
    ...
    private fun addLatestPolyline() {
        if (pathPoints.isNotEmpty() && pathPoints.last().size > 1) {
            ...
            val polylineOptions = PolylineOptions().color(POLYLINE_COLOR).width(POLYLINE_WIDTH).add(preLastLatLng).add(lastLatLng)
            map?.addPolyline(polylineOptions)
        }
    }
